...
class StaticBattleView(QGraphicsView):
    def __init__(self):
        ...
        self.setFixedSize(GAME_W,GAME_H)
...
class StaticBattleWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Static Battle Scene")
        lay=QVBoxLayout(); lay.addWidget(StaticBattleView(), alignment=Qt.AlignCenter)
        cw=QWidget(); cw.setLayout(lay); self.setCentralWidget(cw)
        self.resize(GAME_W+120,GAME_H+120); self.setMinimumSize(GAME_W,GAME_H)
        QShortcut(QKeySequence(Qt.Key_F11), self, self._fs_toggle, context=Qt.ApplicationShortcut)
        QShortcut(QKeySequence(Qt.Key_Escape),self, self.showNormal, context=Qt.ApplicationShortcut)
        QApplication.instance().installEventFilter(self)
    def eventFilter(self,o,e):                             # noqa: D401, ANN001
        if e.type()==QEvent.KeyPress and e.key()==Qt.Key_Escape: self.showNormal(); return True
        return super().eventFilter(o,e)
    def keyPressEvent(self,e):                             # noqa: D401
        if e.key()==Qt.Key_Escape: self.showNormal(); return
        if e.key()==Qt.Key_F11: self._fs_toggle(); return
        super().keyPressEvent(e)
    def _fs_toggle(self): self.showNormal() if self.isFullScreen() else self.showFullScreen()
