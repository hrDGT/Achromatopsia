...
Используется graphic-слоем как «модель».
"""
...
class BattleState:
    ...
    def __init__(
        ...
        # здоровье / мана
        self.player_hp = self.enemy_hp = self.MAX_HEALTH
        self.player_mp = self.enemy_mp = self.MAX_MANA

        # чей ход
        self.turn: str = random.choice(["player", "enemy"])

        # заклинания
        self.spells: Dict[str, Spell] = self._load_spells(spell_file)
        self.enemy_spells: Dict[str, Spell] = self._load_spells(enemy_spell_file)
        ...
        if self.turn == "player":
            ...
            self.enemy_mp = min(self.MAX_MANA, self.enemy_mp + 2)
